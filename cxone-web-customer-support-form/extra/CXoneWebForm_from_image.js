...
async function fetchFromApi(apiUrl, params) {
  ...
  if (!apiUrl) {
    statusEl.textContent = "No API URL configured (use ?api=...)";
    return null;
  }
  ...
  try {
    ...
    const mapped = {
      ...
      booking: {
        id:
          data.booking?.id || data.bookingNumber || params.bookingNumber || "",
...
function updateSummary() {
  const s = {
    ...
    satisfied: document.getElementById("satisfiedYes")?.checked
      ? "yes"
      : document.getElementById("satisfiedNo")?.checked
      ? "no"
      : null,
...
btnSample.addEventListener("click", () => {
  const sample = {
    ...
    notes: "IVR detected billing issue",
  };
...
(function init() {
  ...
  window.addEventListener("resize", () => {
    clearTimeout(rr);
    rr = setTimeout(handleResponsiveAside, 120);
  });
  ...
  document.querySelectorAll('input[name="intent_radio"]').forEach((r) =>
    r.addEventListener("change", () => {
      const h = document.getElementById("intent");
      if (h) {
        const sel = document.querySelector(
          'input[name="intent_radio"]:checked'
        );
        if (sel) {
          h.value = sel.value;
  ...
  const toggles = document.querySelectorAll(
    "#satisfiedYes, #routePhone, #routeChat, #routeSMS"
  );
  toggles.forEach((t) =>
